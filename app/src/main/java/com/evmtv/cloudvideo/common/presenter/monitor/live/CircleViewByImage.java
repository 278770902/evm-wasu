...
public class CircleViewByImage extends FrameLayout {
    ...
    private final int foregroundId, innerColor;
    ...
    public CircleViewByImage(Context context, AttributeSet attrs) {
        ...
        mBackgroundId = array.getResourceId(R.styleable.CircleViewByImage_background, R.drawable.view_control_background);
        foregroundId = array.getResourceId(R.styleable.CircleViewByImage_foreground, R.drawable.view_control_foreground);
        innerColor = array.getColor(R.styleable.CircleViewByImage_inner_backgroundColor, Color.argb(255, 255, 128, 0));
        array.recycle();
        initiation();
    ...
    private void initiation() {
        ...
        imageViewB = new ImageView(mContext);
        imageViewB.setImageResource(R.color.transparent);
        imageViewB.setOnTouchListener(mBackgroundListener);
        imageViewB.setLayoutParams(new ViewGroup.LayoutParams(bgDrawable.getMinimumWidth(), bgDrawable.getMinimumHeight()));
...
//        mInnerCircleRadius = drawable.getMinimumWidth() / 2;//通过png图片获取要画的小圆半径 r
        mInnerCircleRadius = bgDrawable.getMinimumWidth() / 6;//通过png图片获取要画的小圆半径 r
        Log.i("tag", "mInnerCircleRadius=" + mInnerCircleRadius);

        //准备小圆画笔
        innerPaint = new Paint();
        innerPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OVER));
//        innerPaint.setColor(R.color.brown);
//        innerPaint.setAlpha(250);
...
//        center = getWidth() / 2;//在这里得到大圆半径，有可能导致绘图时，小圆在左上角而不是在中间
        ...
        Log.i("tag", "RockerCircleR=" + RockerCircleR + " center=" + center + " mInnerCircleRadius+" + mInnerCircleRadius);
    ...
    private class InnerCircleTouchListener implements OnTouchListener {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            float x = event.getX();
            float y = event.getY();
            Log.i("CircleViewByImage", "InnerCircleTouchListener onTouch event" + event);
            try {
                gestures(action, x, y);
            } catch (Exception e) {
//                throw new NullPointerException("must implement : " + ActionCallback.class.getSimpleName());
                Log.e("CircleviewByImage", e.getMessage());
            }
            return true;
        }
    }
...
//                        + Math.pow((RockerCircleY - (int) y), 2)) >= (RockerCircleR - 120)) {
                if (Math.sqrt(Math.pow((RockerCircleX - (int) x), 2)
...
//                setCircleViewBg("center");
    ...
    private void gestureAction(MyAction myAction, double rad) {
        Log.i("tag", "myAction=" + myAction + " rad=" + rad);
        switch (myAction) {
            case OUTSIDE_MOVE:
                judgeDirection(rad);
                break;
...
//            setCircleViewBg("up");
            ...
            callback.rightMove();
//            setCircleViewBg("right");
        } else {//下
            Log.e("tag", "judgeDirection: 这是下");
            callback.backMove();
//            setCircleViewBg("down");
        }
    }
